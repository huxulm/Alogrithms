...
func TestUndirectGraph(t *testing.T) {
	for _, test := range graphTestCases {
		t.Run(fmt.Sprint(test.name), func(t *testing.T) {
			...
			for _, edge := range test.edges {
				graph.AddWeightEdge(edge[0], edge[1], edge[2])
			}
			...
			if edgeCount != len(test.edges)*2 {
				t.Errorf("Number of edges, Expect: %d Computed: %d", len(test.edges)*2, edgeCount)
			}
			for _, edge := range test.edges {
				if val, found := graph.edges[edge[0]][edge[1]]; !found || val != edge[2] {
					t.Errorf("Edge not found: %d-%d-%d", edge[0], edge[1], edge[2])
				}
			}
		})
	}
